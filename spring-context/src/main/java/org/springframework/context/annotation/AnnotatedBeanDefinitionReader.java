...
public class AnnotatedBeanDefinitionReader {
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		 ...
		 * AnnotatedGenericBeanDefinition包含了类的其他信息，比如一些元信息scope，lazy登等。
		 */
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(annotatedClass);
		/**
		 * 判断这个类是否需要跳过解析。
		 * 通过这个代码可以知道spring是否跳过解析，主要判断类有没有加注解
		 */
		 ...
		 * 处理完成之后processCommonDefinitionAnnotations中依然是把他添加倒数据结构中去
		 */
		 ...
		 * 这里需要注意的是
		 * byName和Qualifier这个变量是Annotation类型的数组。里面存的不仅仅是Qualifier注解
