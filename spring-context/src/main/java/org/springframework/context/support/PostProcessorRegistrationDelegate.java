...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			//这个currentRegistryProcessors放的是spring内部实现了BeanDefinitionRegistryPostProcessor接口的实现类
			...
			//这个地方可以得到BeanFactoryPostProcessor,因为是spring默认在最开始自己注册的
			//为什么要在最开始注册这个呢
			//因为spring的工厂需要解析去扫描等功能
			//而这些功能都是需要在springbean工厂初始化完成之前执行
			//要么要工厂最开始的时候，要么在工厂初始化之中。反正不能再之后
			//因为如果再之后就没有意义，因为那个时候已经需要使用工厂了
			//所以如果spring在一开始就注册了一个BeanFactoryPostProcessor,用来插手SpringFactory的实例化过程
			//在这个地方断点可以知道这个类叫做ConfigurationClassPostProcessor
			//ConfigurationClassPostProcessor这个类能干嘛呢，参考源码
	...
	public static void registerBeanPostProcessors(
		...
		//从BeanDefintionMap中得到所有的BeanPostProcessor
	...
	private static void invokeBeanDefinitionRegistryPostProcessors(
		...
		//只有一条数据即ConfigurationClassPostProcessor
