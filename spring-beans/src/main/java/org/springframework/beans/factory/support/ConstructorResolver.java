...
class ConstructorResolver {
	 ...
	 * new一个对象需要三样东西: 构造方法, 参数类型, 参数值
	 * 这个方法就是确定这三个东西
	 */
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
			@Nullable Constructor<?>[] chosenCtors, @Nullable Object[] explicitArgs) {

		// 实例一个BeanWrapperImpl对象, 前面外部返回的BeanWrapper其实就是这个BeanWrapperImpl, 因为BeanWrapper是个接口
		...
		// 确定参数值列表, argsToUse可以有两种办法设置:
		// (1)通过beanDefinition设置, 比如mybatis里, bd.getConstructorArgumentValues().addGenericArgumentValue()
		// (2)通过xml设置
		...
		// 如果没有已经缓存的构造方法, 则需要去解析构造方法
		if (constructorToUse == null) {
			// Need to resolve the constructor.
			// 判断传入的构造方法是否为空 或者 注入模型是否是构造方法注入
			boolean autowiring = (chosenCtors != null ||
					mbd.getResolvedAutowireMode() == AutowireCapableBeanFactory.AUTOWIRE_CONSTRUCTOR);
			ConstructorArgumentValues resolvedValues = null;

			// 构造方法应该具备的最小参数个数, 如果给了构造方法的参数具体值, 那么这些值的个数就是构造方法的最小参数个数
			// 比如mybatis里给构造方法的参数具体值, bd.getConstructorArgumentValues().addGenericArgumentValue()
			int minNrOfArgs;
			if (explicitArgs != null) {
				...
				// 从bd里拿构造方法的参数值
				// 里面有两个属性, 一个map, 一个list: 应该是 如果参数无序就用list, 如果参数有序就用map
				 ...
				 *     </bean>
				 *
				 *     如果通过spring内部给了一个值, 则表示构造方法的最小参数个数
				 *
				 * minNrOfArgs = 3
				 */
			...
			for (Constructor<?> candidate : candidates) {
				...
				/**
				 * 这个判断别看只有一行代码理解起来很费劲
				 * 首先constructorToUse != null这个很好理解, 前面已经说过首先constructorToUse主要是用来装已经解析过了并且在使用的构造方法
				 * 只有在他等于空的情况下, 才有继续的意义, 因为下面如果解析到了一个符合的构造方法
				 * 就会赋值给这个变量（下面注释有写）。故而如果这个变量不等于null就不需要再进行解析了，说明spring已经找到一个合适的构造方法，直接使用便可以
				...
				// 如果参数个数 小于 构造方法应该具备的最小参数个数, 那么这个构造方法肯定不合适
				...
				if (resolvedValues != null) {
					try {
						...
						// 可以写个代码测试一下: @ConstructorProperties(value = {"xxx", "111"})
						String[] paramNames = ConstructorPropertiesChecker.evaluate(candidate, paramTypes.length);
						if (paramNames == null) {
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							if (pnd != null) {
								// 获取构造方法参数名称列表
								// 假设有一个(String luban, Object zilu), 则paramNames=[luban, zilu]
								paramNames = pnd.getParameterNames(candidate);
							}
						}
						// 获取构造方法参数值列表
						// 这个方法比较复杂, 因为spring只能提供字符串的参数值, 故而需要进行转换, argsHolder所包含的值就是转换之后的值
				 ...
				 * 然后每次循环会把符合条件的typeDiffWeight赋值给minTypeDiffWeight(minTypeDiffWeight = typeDiffWeight)
				 * else if (constructorToUse != null && typeDiffWeight == minTypeDiffWeight)
				 * 第一次循环肯定不会进入这个, 第二次如果进入了这个分支代表什么?
				 * 代表有两个构造方法都符合我们要求? 那么spring就迷茫了(spring经常在迷茫), spring迷茫了怎么办? 把它加到有歧义的构造方法列表里(ambiguousConstructors.add(candidate))
				 * 如果符合条件了, 最后会把ambiguousConstructors清空(ambiguousConstructors = null)
				 * 这也解释了: 为什么如果spring找到两个符合要求的构造方法不直接抛异常, 如果这个ambiguousConstructors一直存在，spring则会在循环外面去抛异常
				 */
				...
				if (typeDiffWeight < minTypeDiffWeight) {
					...
					// 将本次差异量赋值给minTypeDiffWeight, 在循环构造方法的过程中始终保持它是最小的差异量
					minTypeDiffWeight = typeDiffWeight;
					// 说明找到了更合适的, 将ambiguousConstructors进行清空
					...
					// 如果差异量相等, 会添加到 有歧义的构造方法
					ambiguousConstructors.add(candidate);
				}
			}

			/**
			 * ==========循环结束后, 下面检验找到的构造方法是否合适
			 */
			// 循环结束, 如果还没有找打合适的构造方法
			if (constructorToUse == null) {
				if (causes != null) {
			...
			// 如果ambiguousConstructors还存在则抛出异常, 为什么不在上面差异量相等时直接抛异常? 因为可能在后边循环中还能找到更合适的
